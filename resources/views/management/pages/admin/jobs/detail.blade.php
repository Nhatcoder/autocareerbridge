...
@section('content')
                ...
                <div class="page-titles">
                    <nav style="--bs-breadcrumb-divider: '>';" aria-label="breadcrumb">
                        <ol class="breadcrumb">
                            ...
                            <li class="breadcrumb-item active" aria-current="page">{{ __('label.admin.job.detail_job') }}</li>
                        </ol>
                    </nav>
                </div>
            </div>
        ...
        <div class="col-xl-12">
            <div class="card">
                                ...
                                <div class="ul-li">
                                    <ul class="d-flex align-items-center mb-2">
                                        ...
                                        <li class="me-3 me-lg-5"><svg class="me-2" width="24" height="24"
                                            ...
                                            {{ __('label.admin.job.created_at') }}: {{ $data->created_at->format('d/m/Y H:i') }}
                                        </li>
                                        ...
                                        <li><svg class="me-2" width="24" height="24" viewBox="0 0 24 24"
                                            ...
                                            {{ __('label.admin.job.end_date') }}: {{ \Carbon\Carbon::parse($data->end_date)->format('d/m/Y') }}
                                        </li>
                                    </ul>
                                </div>
                ...
                @if ($data->status === STATUS_PENDING)
                    <form class="d-flex justify-content-end border-top" action="{{ route('admin.jobs.updateStatus') }}"
                        id="jobForm" method="POST">
                        @csrf
                        @method('POST')
                        <button type="submit" class="btn btn-primary mx-2 my-3"
                            onclick="setStatus(2)">{{ __('label.admin.job.approve') }}</button>
                        <button type="submit" class="btn btn-danger mx-2 my-3"
                            onclick="setStatus(3)">{{ __('label.admin.job.reject') }}</button>

                        <input type="hidden" name="id" value="{{ $data->id }}">
                        <input type="hidden" name="status" id="statusJob" value="">
                    </form>
                @endif
            </div>
        </div>
    ...
    <script>
        function setStatus(status) {
            document.getElementById('statusJob').value = status;
        }
    </script>
@endsection
